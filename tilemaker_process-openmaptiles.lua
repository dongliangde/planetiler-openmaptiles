...
preferred_language_attribute = "name:zh"
...
additional_languages = { "en", "latin", "de", "fr" }
--------
...
-- 指示应处理节点的那些 OSM 键的列表
node_keys = { "addr:housenumber", "aerialway", "aeroway", "amenity", "barrier", "highway", "historic", "leisure", "natural", "office", "place", "railway", "shop", "sport", "tourism" }
...
poiTags = { aerialway = Set { "station" },
            ...
            railway = Set { "halt", "station", "train_station_entrance", "tram_stop" },
...
function way_function(way)
    ...
    if boundary ~= "" and way:Find("protection_title") == "National Forest" and way:Find("operator") == "United States Forest Service" then
        return
    end
    ...
    if (write_name) and way:Holds("name") then
...
function GetPOIRank(obj)
    ...
    if obj:Find("station") ~= "subway" and obj:Find("light_rail") ~= "yes" and obj:Find("subway") ~= "yes" and obj:Find("monorail") ~= "yes" and obj:Find("train") == "yes" and string.match(obj:Find("name"), "在建")==nil then
        --获取标签去除狄忒
...
-- 处理铁路属性
function SetRailwayAttributes(obj)
    -- 类型
    if obj:Find("railway") ~= "" then
        obj:Attribute("type", "railway")
    end
    -- 废弃型
    if obj:Find("disused:railway") ~= "" then
        obj:Attribute("disused_type", obj:Find("disused:railway"))
    end
    -- 弃型
    if obj:Find("abandoned:railway") ~= "" then
        obj:Attribute("abandoned_type", obj:Find("abandoned:railway"))
    end
    -- 夷为平地
    if obj:Find("razed:railway") ~= "" then
        obj:Attribute("razed_type", obj:Find("razed:railway"))
    end
    -- 建议类型
    if obj:Find("proposed:railway") ~= "" then
        obj:Attribute("proposed_type", obj:Find("proposed:railway"))
    end
    -- 施工类型
    if obj:Find("construction:railway") ~= "" then
        obj:Attribute("construction_type", obj:Find("construction:railway"))
    end
    -- 电压
    if obj:Find("voltage") ~= "" then
        obj:Attribute("voltage", obj:Find("voltage"))
    end
    -- 频率
    if obj:Find("frequency") ~= "" then
        obj:Attribute("frequency", obj:Find("frequency"))
    end
    -- 带电
    if obj:Find("electrified") ~= "" then
        obj:Attribute("electrified", obj:Find("electrified"))
    end
    -- 规矩
    if obj:Find("gauge") ~= "" then
        obj:Attribute("gauge", obj:Find("gauge"))
    end
    -- 使用
    if obj:Find("usage") ~= "" then
        obj:Attribute("usage", obj:Find("usage"))
    end
    -- 服务
    if obj:Find("service") ~= "" then
        obj:Attribute("service", obj:Find("service"))
    end
    -- 名称
    if obj:Find("name") ~= "" then
        obj:Attribute("name", obj:Find("name"))
    end
    -- 架子
    if obj:Find("rack") ~= "" then
        obj:Attribute("rack", obj:Find("rack"))
    end
    -- 是否高铁
    if obj:Find("highspeed") ~= "" then
        obj:Attribute("highspeed", obj:Find("highspeed"))
    end
    -- 操作员
    if obj:Find("operator") ~= "" then
        obj:Attribute("operator", obj:Find("operator"))
    end
    -- 隧道
    if obj:Find("tunnel") ~= "" then
        obj:Attribute("tunnel", obj:Find("tunnel"))
    end
    -- 隧道名称
    if obj:Find("tunnel:name") ~= "" then
        obj:Attribute("tunnel_name", obj:Find("tunnel:name"))
    end
    -- 隧道长度
    if obj:Find("bridge:length") ~= "" then
        obj:Attribute("bridge_length", obj:Find("bridge:length"))
    end
    -- 隧道结构
    if obj:Find("bridge:structure") ~= "" then
        obj:Attribute("bridge_structure", obj:Find("bridge:structure"))
    end
    -- 层
    if obj:Find("layer") ~= "" then
        obj:Attribute("layer", obj:Find("layer"))
    end
    -- 最大速度
    if obj:Find("maxspeed") ~= "" then
        obj:Attribute("maxspeed", obj:Find("maxspeed"))
    end
    -- 最大速度：向前
    if obj:Find("maxspeed:forward") ~= "" then
        obj:Attribute("maxspeed_forward", obj:Find("maxspeed:forward"))
    end
    -- 最大速度：向后
    if obj:Find("maxspeed:backward") ~= "" then
        obj:Attribute("maxspeed_backward", obj:Find("maxspeed:backward"))
    end
    -- 最大速度：倾斜
    if obj:Find("maxspeed:tilting") ~= "" then
        obj:Attribute("maxspeed_tilting", obj:Find("maxspeed:tilting"))
    end
    -- 倾斜
    if obj:Find("incline") ~= "" then
        obj:Attribute("incline", obj:Find("incline"))
    end
    -- 堤
    if obj:Find("embankment") ~= "" then
        obj:Attribute("embankment", obj:Find("embankment"))
    end
    -- 切割类型
    if obj:Find("cutting") ~= "" then
        obj:Attribute("cutting", obj:Find("cutting"))
    end
    -- 货运和客运
    if obj:Find("railway:traffic_mode") ~= "" then
        obj:Attribute("traffic_mode", obj:Find("railway:traffic_mode"))
    end
end
